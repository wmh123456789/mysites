require([
  ...
  esri.config.defaults.io.corsEnabledServers.push('https://developers.arcgis.com/javascript/samples');
  ...
  function updateSandbox() {
    ...
    var relLinks = content.match(/href=".+?"/g).filter(function(url){ return url.indexOf("//") === -1 });
    ...
    if (relLinks) {
      relLinks = relLinks.filter(function(url){ return url.indexOf("//") === -1 });
      relLinks.forEach(function(href){
        var path = href.split("'")[1];
        content = content.replace(path, "../samples/" + sampleFolder + "/" + path);
      });
